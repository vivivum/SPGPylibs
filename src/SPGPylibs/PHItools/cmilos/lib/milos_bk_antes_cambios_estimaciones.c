	...
	if(!RFS){
		do{
			...
			if(CLASSICAL_ESTIMATES && !RFS){
				...
				//Escribimos en fichero las estimaciones clásicas (en S0 se almacena Blos)
				fprintf(fichero_estimacioneClasicas,"%e %e %e %e %e\n",initModel.B,initModel.gm,initModel.az,initModel.vlos,initModel.S0);
				initModel.S0 = INITIAL_MODEL_S0;
...
//		printf("m : %f \n",m);
	...
	tan_gamma = fabs(tan_gamma) > 1e-5 ? tan_gamma : 0;
	
	gamma_rad = atanf(tan_gamma); //gamma en radianes
	
	gamma_rad = fabs(gamma_rad) > 1e-5 ? gamma_rad : 0;
	...
	B_aux = fabs(Blos/cos(gamma_rad))*1.5;
